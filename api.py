...
@app.route("/clientdetails/<variable>", methods=["GET", "POST"])
def show_client_details(variable): 
    cid = execute_query('SELECT distinct id FROM company t where company_name = \'{}\';'.format(variable), 'select')
    print(cid)
    #cid = cid[0][0]
    print('+++++++++++',cid[0], cid[0][0], '++++++++')
    file_path_combined, analysis_dict = calculate_combine_income(cid[0][0])
    session["file_path_combined"] = file_path_combined
    return render_template('outputCalc.html')

@app.route("/listClients", methods=["GET", "POST"])
def list_data(): 
    clients = execute_query('select distinct company_name  from company where company_name is not null;', 'select')
    clients = [i[0] for i in clients]
    print(clients)
    return render_template('list_companies.html', clients=clients)
