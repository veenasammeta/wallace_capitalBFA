...
# host_name = 'localhost'
host_name = '74.208.242.179'
# database="wallace_db", user='postgres', password='wallace123', host=74.208.242.179, port= '5432'
conn = psycopg2.connect( database="wallace_db", user='postgres', password='wallace123', host=host_name, port= '5432')
dic_summary = {}
...
def calculate_combine_income(company_id):
    ...
    if total_business_monthly_income is None:
        total_business_monthly_income = 0
    if total_personal_monthly_income is None:
        total_personal_monthly_income = 0
    print('*****', total_business_monthly_debt, total_person_monthly_debt , '*****')
    if total_business_monthly_debt is None and total_person_monthly_debt is None or (total_person_monthly_debt == 0 and total_business_monthly_debt == 0):
        total_business_monthly_debt = 1
        total_person_monthly_debt = 0
    # Gdscr 
    ...
    # DTO
    dto_query = 'select  coalesce( coalesce (sum(coalesce(total_owed)),0)/sum(coalesce(credit_limit,1)) *100, 0)  from person_debt_info pdi where debt_type= \'revolving\' and company_id= {};'.format(company_id)
...
def persondata_to_db(cid, person_dict):
    ...
    for iter in range(1, max_inc+1):
        ...
        person_inc_query =  prepare_query('insert into person_credit_info values ({}, {}, {}, {} , {}, {}, {}, {});commit;', [max_val, cid, creditYear, AGI[1:].replace(',' , ''), ScheduleC, K1s, Compensation, Deduction])
...
def find_estimates(form_data):
    try:
        personal_income = float(form_data['personal_income'])
    except Exception as err:
        personal_income = 0

    try:
        business_income = float(form_data['business_income'])
    except Exception as err:
        business_income = 0
            
    try:
        total_person_monthly_debt = float(form_data['total_person_monthly_debt'])
    except Exception as err:
        total_person_monthly_debt = 0

    try:
        total_business_monthly_debt = float(form_data['total_business_monthly_debt'])
    except Exception as err:       
        total_business_monthly_debt = 0         
    ...
    try:
        RequestedFund = float(form_data['RequestedFund'])   
    except Exception as err:
        RequestedFund = 0                             
            
    est = estimate_monthly_pmt( PrimeRate + AdditionalRate , tenure, RequestedFund )
    gdscr = estimate_gdscr(personal_income, business_income, total_person_monthly_debt, total_business_monthly_debt , est)
    total_income = personal_income + business_income
    total_debt = total_person_monthly_debt + total_business_monthly_debt
